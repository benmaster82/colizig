<#
  bench_ab.ps1 - fair A/B: colibri vs colizig on the same real checkpoint.
...
# --- resolve prompt -> token ids with colizig's tokenizer (== colibri's) ---
...
if ($haveColibri) {
  ...
  Write-Host "`n=== colibri: not found at $Colibri - running colizig only ===" -ForegroundColor DarkYellow
}

# ============================ colizig ============================
Write-Host "`n=== colizig ===" -ForegroundColor Yellow
...
"{0,-16}{1,14}{2,14}" -f "",              "colibri",   "colizig"
...
Write-Host "  (colizig WS includes reclaimable mmap page cache; its private" -ForegroundColor DarkGray
Write-Host "   allocation is about 7 GB - see 'tracked peak' from: colizig benchmark)" -ForegroundColor DarkGray
Write-Host ""
Write-Host ("output match : {0}" -f $match)
if ($haveColibri) { Write-Host ("  colibri    : $cCont") }
Write-Host ("  colizig    : $zGreedy")
